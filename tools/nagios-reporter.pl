...
my $maildomain  = 'gmail.com';                     # Fill these in!
my $mailfrom    = 'robert.h3x' . $maildomain;         # Fill these in!
my $mailto      = 'robert.h3x@' . $maildomain;    # Fill these in!
my $timeout     = 30;
my $mailsubject = '';
my $mailbody    = '';
...
my $webuser     = 'monitoring';            # Set this to a read-only nagios user (not nagiosadmin!)
my $webpass     = 'monitoring';        # Set this to a read-only nagios user (not nagiosadmin!)
...
sub report_monthly {
  ...
  if ( $locale eq "de_DE" ) {
    ...
    $mailsubject = "Nagios alerts on DC for month $month_name ($repsmonth/$repsyear)";
  }
  return 0;
}
...
sub report_weekly {
  ...
  if ( $locale eq "de_DE" ) {
    ...
    $mailsubject = "Nagios alerts on SJC for CW${week_number} ($repsday/$repsmonth/$repsyear ${repshour}:00h to $repeday.$repemonth.$repeyear ${repehour}:00h)";
  }
...
sub report_daily {
  ...
  if ( $locale eq "de_DE" ) {
    ...
    $mailsubject = "Nagios alerts on SJC of $prev_day_name ($repsday/$repsmonth/$repsyear) ${repshour}:00h to $day_name ${repehour}:00h";
  }
  return 0;
}
